...
func get_info() -> String:
	...
	if type == 0:
		return ("{0}, {1}\n".format([coords,move_to_value])+empty_compile+reach_compile)
	elif type == 1:
		return ("{0}, {1}\n".format([coords,move_to_value])+enemy_compile+attack_copile+boost_text+reach_compile)
	elif type == 2:
		return ("{0}, {1}\n".format([coords,move_to_value])+player_compile+attack_copile+boost_text)
	elif type == 3:
		return ("{0}, {1}\n".format([coords,move_to_value])+wall_text+empty_compile)
	else:
		return ""
