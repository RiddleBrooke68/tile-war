...
func update():
	if claim != null:
		...
		if not claim.claim_dead:
			if claim.claim_dangered == 0 and claim.claim_panel_normal != null:
				panel.texture = claim.claim_panel_normal
			elif claim.claim_dangered > 0 and claim.claim_panel_danged != null:
				panel.texture = claim.claim_panel_danged
